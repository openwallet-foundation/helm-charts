...
| `controller.userVariableSubsitution`                    | The placeholder for the user variable substitution file that can be used to add substitution variables                                                                                               | `""`                               |
...
| `service.port`                                          |                                                                                                                                                                                                      | `5000`                             |
| `service.sessionAffinity`                               | Control where client requests go, to the same pod or round-robin. Enables sticky sessions for websocket support.                                                                                     | `ClientIP`                         |
| `service.sessionAffinityConfig.clientIP.timeoutSeconds` | Timeout in seconds for the session affinity                                                                                                                                                          | `10800`                            |
...
| `acapy.image.repository`                              |                                                                                                                     | `openwallet-foundation/acapy-vc-authn-oidc-agent` |
| `acapy.image.pullPolicy`                              |                                                                                                                     | `IfNotPresent`                                    |
| `acapy.image.pullSecrets`                             |                                                                                                                     | `[]`                                              |
| `acapy.image.tag`                                     | Overrides the image tag which defaults to the chart appVersion.                                                     | `2.4.0`                                           |
...
| `acapy.argfile.yml.auto-respond-presentation-request` | Automatically respond to Indy presentation requests with a constructed presentation if a corresponding credential can be retrieved. Default: false.                                                                                             | `true`             |
| `acapy.argfile.yml.auto-verify-presentation`          | Automatically verify a presentation when it is received. Default: false.                                                                                                                                                                        | `true`             |
| `acapy.argfile.yml.debug-webhooks`                    | Enable webhook debug logging. Default: false.                                                                                                                                                                                                   | `true`             |
...
| `acapy.argfile.yml.preserve-exchange-records`         | Keep credential exchange records after exchange has completed. Default: false.                                                                                                                                                                  | `false`            |
...
| `acapy.argfile.yml.wallet-local-did`                  | If this parameter is set, provisions the wallet with a local DID from the '--seed' parameter, instead of a public DID to use with a Hyperledger Indy ledger. Default: true.                                                                     | `true`             |
...
| `acapy.argfile.yml.wallet-type`                       | Specifies the type of wallet provider to use. Supported types are 'askar' and 'askar-anoncreds'. Use 'askar-anoncreds' for WebVH support.                                                                                                       | `askar-anoncreds`  |
| `acapy.argfile.yml.plugin`                            | List of ACA-Py plugins to load                                                                                                                                                                                                                  | `[]`               |
| `acapy.plugin-config.yml`                             | Plugin configuration file for ACA-Py plugins                                                                                                                                                                                                    | `{}`               |
| `acapy.ledgers.yml`                                   |                                                                                                                                                                                                                                                 | `{}`               |
...
| Name                                 | Description                                           | Value  |
| ------------------------------------ | ----------------------------------------------------- | ------ |
| `acapy.postgres.enabled`             | Switch to enable or disable the PostgreSQL helm chart | `true` |
| `acapy.postgres.persistence.enabled` | Enable PostgreSQL data persistence using PVC          | `true` |
| `acapy.postgres.commonLabels`        | Apply common labels to postgres resources             | `{}`   |
...
| `mongodb.targetPlatform`                     | Target platform for MongoDB deployment. Set to "openshift" for OpenShift compatibility, leave empty for standard Kubernetes. | `""`                                      |
| `mongodb.auth.enabled`                       | Enable MongoDB authentication                                                                                                | `true`                                    |
| `mongodb.auth.rootUsername`                  | MongoDB root username                                                                                                        | `admin`                                   |
| `mongodb.auth.existingSecret`                | Existing secret with MongoDB credentials (key: `mongodb-root-password`)                                                      | `{{ printf "%s-mongodb" .Release.Name }}` |
| `mongodb.customUser.name`                    | Name of the custom user to be created                                                                                        | `vcauthn`                                 |
| `mongodb.customUser.database`                | Name of the database to be created                                                                                           | `vcauthn`                                 |
| `mongodb.customUser.existingSecret`          | Existing secret containing the custom user password                                                                          | `{{ printf "%s-mongodb" .Release.Name }}` |
| `mongodb.customUser.secretKeys.name`         | Key name for username in existing secret                                                                                     | `mongodb-user`                            |
| `mongodb.customUser.secretKeys.password`     | Key name for password in existing secret                                                                                     | `mongodb-password`                        |
| `mongodb.customUser.secretKeys.database`     | Key name for database in existing secret                                                                                     | `mongodb-database`                        |
| `mongodb.commonLabels`                       | Add labels to all the deployed resources                                                                                     | `{}`                                      |
| `mongodb.podLabels`                          | Add labels to the MongoDB pods (required for network policy)                                                                 | `{}`                                      |
| `mongodb.replicaSet.enabled`                 | Enable MongoDB replica set mode (standalone if false)                                                                        | `false`                                   |
| `mongodb.service.type`                       | MongoDB service type                                                                                                         | `ClusterIP`                               |
| `mongodb.service.port`                       | MongoDB service port                                                                                                         | `27017`                                   |
| `mongodb.containerSecurityContext`           | Set MongoDB container security context                                                                                       | `{}`                                      |
| `mongodb.podSecurityContext`                 | Set MongoDB pod security context                                                                                             | `{}`                                      |
...
| `redis.containerSecurityContext.allowPrivilegeEscalation` | Set Redis containers' Security Context allowPrivilegeEscalation                        | `false`             |
| `redis.containerSecurityContext.readOnlyRootFilesystem`   | Set Redis containers' Security Context readOnlyRootFilesystem                          | `true`              |
| `redis.containerSecurityContext.capabilities.drop`        | List of capabilities to be dropped in Redis containers                                 | `["ALL"]`           |
| `redis.persistence.enabled`                               | Enable Redis data persistence using PVC. If disabled, an emptyDir volume will be used. | `false`             |
| `redis.persistence.storageClass`                          | PVC Storage Class for Redis data volume                                                | `""`                |
| `redis.persistence.accessModes`                           | PVC Access Mode for Redis volume                                                       | `["ReadWriteOnce"]` |
| `redis.persistence.size`                                  | PVC Storage Request for Redis volume                                                   | `8Gi`               |
| `redis.persistence.annotations`                           | Annotations for the PVC                                                                | `{}`                |
...
| `externalRedis.host`                   | External Redis host list. Accepts a single host or comma-separated hosts. Entries may be provided as host:port, otherwise externalRedis.port is appended automatically. Cannot be used together with redis.enabled=true. | `""`       |
| `externalRedis.port`                   | External Redis port appended to externalRedis.host entries that do not already include a port                                                                                                                            | `6379`     |
