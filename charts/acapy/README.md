...
Configure ACA-Py webhook delivery.
Set `webhook.url` to send webhooks to a plain URL (evaluated as a template, so parent charts can compute it dynamically).
Set `webhook.existingSecret` to load a pre-composed `ACAPY_WEBHOOK_URL` from a Secret (takes precedence over `webhook.url`).
If neither is set, webhooks are disabled.

To include an API key in the URL, either bake it into `webhook.url` directly as `<url>#<apikey>`,
or store the full composed value in a Secret and reference it with `webhook.existingSecret`.

| Name                                    | Description                                                                                                                                             | Value                 |
| --------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| `webhook.url`                           | Webhook callback URL. Evaluated as a template. Set ACAPY_WEBHOOK_URL to this value.                                                                     | `""`                  |
| `webhook.existingSecret`                | Name of an existing Secret containing a full ACAPY_WEBHOOK_URL value (including any API key). Takes precedence over webhook.url.                        | `""`                  |
| `webhook.secretKey`                     | Key in webhook.existingSecret containing the ACAPY_WEBHOOK_URL value.                                                                                   | `ACAPY_WEBHOOK_URL`   |
| `ledgers.yml`                           |                                                                                                                                                         | `{}`                  |
